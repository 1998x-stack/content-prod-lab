...
```bash
python scripts/lo_convert_to_pdf.py input.docx --out_dir /tmp/out
```
...
```bash
python scripts/html_to_pdf.py input.html -o /tmp/out.pdf --format letter
```
...
```bash
python scripts/md_to_pdf.py input.md -o /tmp/out.pdf --pdf_engine xelatex
```

## LaTeX -> PDF

```bash
python scripts/latex_to_pdf.py input.tex -o /tmp/out.pdf
```
