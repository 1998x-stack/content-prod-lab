...
- If you are in an offline environment, you can fall back to the Python helpers:
  - Form fill (best-effort): `python scripts/pdf_edit.py fill-form in.pdf --values values.json -o out.pdf`
  - Text extraction: `python scripts/pdf_extract.py text in.pdf --method pdfplumber`
...
```bash
node js/extract_form_fields.mjs --input in.pdf
```
...
```bash
node js/extract_text_pdfjs.mjs --input in.pdf > text.txt
```
